...
- §1.4.5 (write user bytes verbatim) is the constitutional backing: a double-claim means the same source byte appears in two SyntaxSpans, which can cause the emitted visible text to either duplicate or drop that byte depending on which span wins.
