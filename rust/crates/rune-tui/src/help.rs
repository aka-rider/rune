...
//! `## Explorer`/`## Open Tabs` are each built by iterating
...
//! already read (one source of truth, `KeyPattern::label`/
//! `global::leader_label` the shared display-string chokepoints).
//!
...
/// The `## Leader` section (plan WP5.S9): generated by reflection over
...
fn push_editor_section(out: &mut String) {
    ...
    const ROWS: &[(&str, &str)] = &[
        ...
        ("PageUp / PageDown, ^U", "page up / down"),
        (
...
mod tests {
    ...
    /// Regression for WP5.S9: the Leader section is generated by reflection
    /// over `LEADER_BINDINGS`, not a hand-maintained list (§12) — every
    /// entry's help label and `␣`-prefixed key label must appear.
    #[test]
