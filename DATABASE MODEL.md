...
ENDGAME:>
    ...
    underneath:
        Separate 1x Reader 1x Writer connections.
        
         `sqlite-watcher`'s `State`/`TableObserver` split as the shape for your notification API — it's designed to be [patched into an existing connection](https://docs.rs/sqlite-watcher/latest/sqlite_watcher/), so its in-process machinery can sit behind cross-process trigge
        
        STRICT DISCIPLINE: read/write operations properly assigned to each operation
        Priorities: 
...
CONTEXT:
WHY: Simplicity of the ACID dabase app throughout the whole app.
Show me your code and I will stay mystified, show me your data and I shall know your code.
You define the shape of the whole app. Your task is very complex.
You must present the state of the whole app DOWN-TOP.
THUS, YOU MUST validate any potential solution TOP-DOWN first: will it work correctly? will UX be good?
Domain-driven design: separate and conquer
