...
## go editor/display — table rendering defects, verified against the Rust port (recorded 2026-07-28, markdown-table-rendering plan WP6)

**Status: not fixed — the Rust port is the correct reference for these cases.** Recorded so a future reader doesn't mistake Go's output for the intended behavior. The Rust side (`crates/rune-md`) uses comrak's real table AST and GFM-spec-conformant parsing; Go's own table rendering (`pkg/editor/display/{markdown_table.go, markdown_table_format.go, markdown_table_layout.go, markdown_table_util.go, table_rows.go}`) predates that port and has several verified defects, each demonstrated in `scripts/parity/fixtures/tables-divergent.md` (excluded from the `parity-grid` gate, `scripts/parity/grid.sh`'s `excluded_reason`; write-up also in `scripts/parity/README.md`'s "Known divergences").

- **Inverted alignment.** goldmark's `TableAlignment` enum is `AlignLeft = iota + 1` (Left=1, Right=2, Center=3, None=4). `markdown_block.go:230-234` and `markdown_table_format.go:190-220` cast that raw value into a renderer switch that reads `0=left/1=center/2=right` — off by one against goldmark's own numbering — so a `:---` column (goldmark `AlignLeft`=1) renders centred and a `:---:` column (goldmark `AlignCenter`=3, reads as `2=right` in the switch, missing the actual 1=center case entirely by two) renders as Go's `default` (left) case. Net effect: `:---` centres, `:---:` renders left.
- **`strings.Split(line, "|")` cell splitting** (`markdown_table_util.go:145`). No escaping awareness: a `\|` inside a cell is still a split point, so an escaped pipe misaligns a row's columns instead of staying part of one cell's content. No column-count enforcement either: a ragged row with more cells than the header keeps its extra cell instead of truncating to the established column count (GFM's own rule, which comrak implements).
- **Container-prefix leakage.** The same naive line-splitting doesn't strip a blockquote's `> ` or a list item's indentation before looking for `|`-delimited cells, corrupting the first cell of a table nested inside either container.
- **Per-rune, not per-grapheme, cell width.** Go's table cell width accounting measures each rune independently rather than each grapheme cluster, so a multi-codepoint ZWJ emoji sequence (e.g. a 7-codepoint family emoji) is counted as several cells wide instead of one — the same class of defect already fixed on the Rust side for non-table text (see repo-root `TODO.md`'s resolved ZWJ/skin-tone-modifier entry).
- **CJK-heavy table rows hit the vendored-renderer TAB-padding defect** documented earlier in this file ("a long CJK-wrapped row's trailing padding is literal TAB bytes, not spaces") — the same root cause, not a table-specific one, but it makes any CJK-heavy table an unreliable `parity-grid` fixture for the same reason `cjk.md` already is.
- **Grid-fit threshold arithmetic is inconsistent with itself.** `computeMinGridWidth` (`markdown_table_layout.go:72-81`) returns `Σw + 4n − 1`, but an actually rendered Grid row is `Σw + 3n + 1` wide (`markdown_table_format.go:168-227`) — the two formulas agree only at exactly 2 columns. Not independently gated (no fixture straddles it — `scripts/parity/fixtures/tables-narrow.md` is sized so both formulas land on the same side of their own threshold, per the Rust-side plan's Assumption A1), but recorded here since it's a Go-side defect in its own right, not just a Rust/Go divergence.
