...
- [ ] **After a COMPLETED merge, ⌘Z past the working-form install then ⌘S passes the CAS silently while the footer shows ⇄.** Sync's unwind override truthfully reports `Diverged` (the journal position fell below the resolve observation), but the save-CAS baseline (`DocDb::expect_obs`) legitimately points at the current disk bytes the merge adopted, so the save commits with no guard while the chrome claims divergence. Candidate design: an unwind-aware CAS baseline in `rune-db` — materialize prepare could refuse when the journal position sits below the expect observation's correlated seq. Auto-abandoning the resolve row on unwind is WRONG: it would break the undo→redo roundtrip's re-classification (redo past the install must find the resolve row again).

- [ ] **The Open Tabs renderer never truncates document names to the pane width.** `opentabs::draw` budgets height only — a name wider than the left column overflows its rect instead of being clipped at a grapheme boundary. Pre-existing before the diverged-sync `⇄` column landed; that fixed column narrows the room for names by one further cell at every width. The fix belongs at the shared width-measure chokepoint the divider row already uses — `crate::width::truncate_to_width`/`display_width` — applied to the name span with the fixed prefix columns (cursor, digit, pin, dirty, sync) subtracted from the budget.

- [ ] **`crates/rune-tui/src/footer.rs` is over the 500-line file budget (506 lines).** The persistent diverged-sync `⇄` marker (`sync_marker` + its rendered-row test) tipped a file that sat at 437. Named split candidate: move the `mod tests` block to a sibling test file the way `search/keys/tests.rs` was split out — the module is pure render logic and its tests already drive it only through `footer_text`/`draw`.
...
- [ ] **`crates/rune-fuzz/src/driver/mod.rs` is over the 500-line file budget (502 lines).** Reconciling PR #53 (store-backed sessions: `Action::DivergeDisk`/`DeliverDb`/`DeliverDbAll`) against PR #49 (issue #37: `Action::HighlightTree`) landed both `Action` arms' driver dispatch in the same file. Not split here — the merge's own scope is reconciling the two branches' `Action::Highlight`/`Action::HighlightTree` reply construction into `step_exec::highlight_step`/`highlight_tree_step`, not this file's overall size. A follow-up should look at moving the store-backed arms (`DivergeDisk`/`DeliverDb`/`DeliverDbAll`) into `store_ops.rs`, which already owns the rest of that surface.

- [ ] **`commands/edit.rs`'s `undo`/`redo` bare-return with no user feedback when there is nothing to undo/redo** (journal at a boundary) or the active document is `ReadOnly::Reading` — a silent input swallow, the same class `MERGE-KEY-FEEDBACK` was written to police, just outside merge mode. The fix direction is a `messages::info` refusal (a "nothing to undo"-style line, consistent with the merge resolver's own refusal copy) at both the boundary and the read-only gate, plus the same audit for `redo`. Root-caused while fixing `MERGE-KEY-FEEDBACK`'s harness duplicate-post blindness; not fixed here — out of scope for that harness-only change.

- [ ] **`MERGE-KEY-FEEDBACK` fires on `⌥⌘Up`/`AddCursorAbove` dispatched into an active merge on a doubly-nested conflict region** — issue #54. `merge/keys.rs::intercept` neither resolves the key nor posts its `MERGE_KEY_HINT` warning, so buffer, cursors, scroll, merge state, status and the message log all come out unchanged. Caught by `make test-fuzz` on `rr` at `e78f2771` (case 499/512 of `human_session`, seed `cc 1a6f966a05ef047c36bb575954b0a23fc6f17d09a45621df6b7e09b568edd2ab`); the failing script and report are recorded verbatim in the issue. The repro is deliberately NOT checked into `crates/rune-fuzz/repros/` — that directory's README requires a script land only in the same commit as the fix that makes it pass, and `tests/replay.rs` was confirmed to go red with it present. Distinct from `repros/merge-key-feedback-01.rune` (which stops at the second `^M` and uses no cursor keys) and from the `[`/`]` single-conflict nav gap recorded below; likely the same root cause as the `undo`/`redo` entry above — the merge-mode key dispatch has no refusal-feedback path for the commands it does not handle.
...
- [ ] **A recovery-store `AppendEdit` can be replayed against a stale, shorter replica once a real fuzz session reaches `MergeState::Active`.** Now that the session fuzzer wires a real in-memory `Store` behind its fixture `App` and can genuinely diverge/enter/exit a merge attempt (issue 35), `make test-fuzz` intermittently surfaces a writer-thread replay error — `edit out of bounds: [start,end) len=0` — that degrades the store (`materialize_ack::on_store_failure`) mid-session; observed on a shrunk case combining typed edits, an undo, a diverge-enter-exit merge attempt, a save-arm, and a second diverge-enter-exit merge attempt on the same document. The store's own safety net catches it correctly (halts with a surfaced error and a persistent degraded banner, never corrupts the recovery journal or panics), so no user bytes are at risk — but the underlying mismatch between the app's local buffer length and the store's replayed replica length, reachable only through this specific merge-install + later-edit interleaving, is real and needs its own investigation in `rune-db`'s replay path or `rune-tui`'s `db_enqueue`/merge-landing sequencing. Reproduces deterministically through `rune_fuzz::driver::run` on the fixed action list `proptest` shrinks to (see the session fuzzer's own gitignored `artifacts/proptest-regressions/human_session.txt` after a `make test-fuzz` run with `cluster_merge` enabled); not fixed here — out of scope for the fuzzer-plumbing work that surfaced it.

- [ ] **`MERGE-KEY-FEEDBACK` can fire on an ordinary, harmless repeat of `merge/keys.rs`'s conflict-navigation keys (`[`/`]`) when only one conflict block exists.** `merge::resolve::nav`'s own docs say both directions land back on the sole remaining block, so a second `[`/`]` press in that shape changes nothing observable (same cursor, same scroll, same status) — correctly caught by the invariant's "no observable trace" rule even though nothing is actually wrong; the resolver gives no distinguishing feedback (no bump, no wrap indicator) for "you're already here." `crates/rune-fuzz/src/generate/cluster.rs`'s `cluster_merge` sidesteps this by generating exactly one resolve key per attempt rather than fixing the underlying gap, which still exists for any real user who presses the same nav key twice on a single-conflict merge.
