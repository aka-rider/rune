...
- [ ] **`MERGE-KEY-FEEDBACK` also fires for ⌘Z (Undo) dispatched while merge is `Active`**, distinct from the already-recorded repeat-nav-key instance below: the keymap resolves `⌘Z` to `Some(Undo)` even with merge active and Editor focus, and dispatch leaves buffer, cursors, scroll, merge state, and status all unchanged — a silent swallow with no feedback, not a crash or data-loss path. Surfaced by `RC=300 make test-fuzz` on the `fuzz-merge-store-undoseq` branch (issue 35 undo-seq fix), confirmed unrelated to that fix by replaying the pinned artifact against the pre-fix commit (`6036d4cc`) with identical output. Artifact: `crates/rune-fuzz/artifacts/merge-key-feedback-09a10bd4/{report.txt,script.rune}` (gitignored). Not chased here — out of scope for the undo-seq writer-resolution fix.
