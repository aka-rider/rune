...
- **Where** (recomputed from the live tree with `wc -l`; comment purge below will change these numbers):
  ...
  - `crates/rune-tui/src/db_enqueue.rs` — 542 (newly over — the writer-authority undo rework (#119/#120) added `resolve_drift`, the sibling/reload drift-translation chokepoint every `AppendEdit` now funnels through; split candidate: move `resolve_drift` plus its doc comment to a sibling `db_drift.rs`, leaving the enqueue/`move_undo_pos`/`rebase_move` wiring here)
  - `crates/rune-tui/src/db_ack.rs` — 503 (back over 500 — the same rework's `bind_document_row` grew to compute `diverged`/`synced_content` for a same-row bind; no split candidate identified yet, the function is already the file's one long chokepoint)
