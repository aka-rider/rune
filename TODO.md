...
### Undoing past a merge's own install edit falsely marks it complete and advances the save baseline
- **Where**: `crates/rune-tui/src/merge/ranges.rs`'s `rederive_after_jump` (the `Resolution::HandEdited` fallback arm, and its trailing `if session.unresolved_count() == 0 { super::exit_in_place(app); }`); called from `crates/rune-tui/src/commands/edit.rs`'s `resync_after_journal_jump` on every undo/redo while a merge is `Active`. Tracked as issue #106 (`MERGE-NO-INSTANT-REDIVERGENCE`, pre-existing, reproduced on a detached pre-diff-view-plan worktree — not caused by this plan's work).
- **Wrong**: undoing past the journal step that installed a conflict-bearing merge (`landing.rs`'s `install_whole_range`) reverts the buffer to its pre-install bytes. `rederive_after_jump` remaps each conflict's byte range through the undo's delta, reads whatever bytes now sit there, and — since they match neither the conflict's `ours` nor `theirs` text — falls back to `Resolution::HandEdited`, which `Resolution::is_resolved()` counts as resolved. With every conflict "resolved" this way, `unresolved_count() == 0` and `rederive_after_jump` calls `exit_in_place`, which takes the terminal-success arm: it sets `last_sync = BufferAhead` and advances the save-CAS baseline (`landing::advance_expect_obs`) to the disk bytes the merge originally read — even though the buffer the user is now looking at is the pre-merge content, with no trace of any resolution. A following ⌘S publishes over the disk's `theirs_obs` state without a CAS refusal, though the user never actually reviewed or resolved anything in this state. This precise trigger (found working WP9 of the diff-view plan, posted as a comment on #106) is a stronger claim than #106's original "feedback/UX, not data-loss" framing.
- **Instead**: `rederive_after_jump` needs to tell "the user hand-edited live conflict content" (a real, forward `HandEdited`) apart from "the jump removed the block's own installing edit" (the block's identity is gone, not resolved) — for example by tracking each conflict's originating edit sequence and treating a jump that crosses back over it as `Unresolved` rather than falling through to `HandEdited`.
- **Done when**: undoing past a merge's own install edit reopens its conflicts as `Unresolved` (never advances `expect_obs`, never auto-exits as completed), pinned by a `merge_resync_guard.rs` test that undoes past the install boundary and asserts the merge stays `Active` with the conflict `Unresolved`; issue #106 closed in the same commit.
...
- **Where** (recomputed from the live tree with `wc -l`; comment purge below will change these numbers):
  ...
  - `crates/rune-tui/src/global.rs` — 850 (grew from 793: WP6 `DIFF_BINDINGS` registration into `claimants_across_pane_tables` plus the new bidirectional collision guard test)
  ...
  - `crates/rune-tui/tests/diff_view.rs` — 574 (test file; newly over — the diff-view plan's own test suite grew through WP3-WP8: layout/alignment/intraline tests plus WP6's verb/chord/click tests all landed here; split candidate: move the verb and chord tests, `take_theirs_makes_the_region_same_and_undoes_in_one_step` through `click_in_the_left_pane_moves_the_right_pane_caret_to_the_aligned_line`, to a sibling `diff_view_verbs.rs`, leaving the layout/alignment/intraline tests here)
  - `crates/rune-fuzz/src/generate/cluster.rs` — 585 (grew further from the recorded 505: WP9's merge-chord rework of `cluster_merge`'s own doc comment; split candidate unchanged)
