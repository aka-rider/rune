...
- [ ] **`crates/rune-tui/src/search/keys.rs` is over the 500-line file budget (639 lines).** Plan WP6's history browsing (`history_prev`/`history_next`/`browse_needle`, plus their own test module) landed on top of WP3/WP4's already-substantial keystroke handling and its test suite, pushing the file well past the ceiling. Not split here to stay inside WP6's own scope (no drive-by restructuring of a file two prior work packages already own). A follow-up should extract the history-browsing functions and their tests into a sibling `search/history_keys.rs` — `super::{fuzzy_filter, recompute}` are already the only things it needs from `search/mod.rs`, and `handle_key`'s `KeyCode::Up`/`Down` arms would call through the new module exactly like `keys::advance` already does for Enter/Shift+Enter today.
