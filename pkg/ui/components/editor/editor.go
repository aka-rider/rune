...
func (m Model) update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case placementTickMsg:
		// Deferred image placement: emit the Raw sequence now.
		// BubbleTea processes RawMsg via p.execute() BEFORE p.render(),
		// and since View() hasn't changed, the renderer skips → image survives.
