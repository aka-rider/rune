...
// raiseDeletedGuard arms pendingDeleted and raises the GuardDeleted footer
...
func (m Model) raiseDeletedGuard(docID int64, path string) Model {
	m.pendingDeleted = pendingDeleted{active: true, path: path, docID: docID}
	m.err = nil
...
func (m Model) handleProbeResult(msg probeResultMsg) (Model, tea.Cmd) {
	...
	if msg.missing {
		if !m.footer.InGuard() && !m.pendingDeleted.active {
