...
// evictIntent carries the state a raised dirty-eviction guard must survive
// across the async Save→FileSavedMsg round-trip (§5.5): victim identifies
// the tab to close and pendingOpenPath is the file to open once the victim
// is dealt with; requestID correlates the background save ack. Zero value =
// no pending eviction. Lives at guardState.evict (A4: migrated from the
// former Model.pendingDataLoss{kind:actionEvict}).
type evictIntent struct {
	active          bool
	victim          opentabs.TabHandle // eviction target
	pendingOpenPath string             // file to open after eviction
	requestID       string             // correlates evict-save ack
}
...
func (m Model) enforceTabLimit(docID int64, path string) (Model, tea.Cmd, bool) {
	...
	if dirty {
		...
		// pre-A4 `pendingDataLoss = pendingDataLoss{kind: actionEvict, ...}`
		...
		m = m.raiseGuardPrompt(guardDirtyEvict)
...
func (m Model) evictSave() (Model, tea.Cmd) {
	...
	m.guard.evict.requestID = requestID
...
func (m Model) evictDiscard(action evictIntent) (Model, tea.Cmd) {
...
func (m Model) evictSaveAck() (Model, tea.Cmd) {
	victim := m.guard.evict.victim
	pendingPath := m.guard.evict.pendingOpenPath
