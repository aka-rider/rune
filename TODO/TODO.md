...
Scope: the Rust implementation at the repo root (`crates/…`).
...
`CLAUDE.md`'s House Rules bullet on comment citations was ambiguous about whether a bare filename (no `:line`) counts as a banned "file path" — read literally, it did, yet the codebase carries roughly 238 bare-filename references in comments (`palette.rs`, `cluster.rs`, …), 218 of which predate `fix-fuzz`. The rule's own stated rationale is ROT: a `path:line` citation rots the moment either file moves (verified repeatedly on this branch), while a bare filename or module path is far less rot-prone — it survives line churn, only breaking on a rename. The rule text is now amended to say this explicitly: `path:line` citations are banned outright, a bare filename/module path is tolerated but discouraged and should be replaced with a description of the invariant when touched. The existing ~238 references were deliberately **not** mass-edited to close this finding — the same "needs a judgement call per comment" reasoning applies. Replace opportunistically when touching the surrounding code, per the amended rule.

## rust port — vfs primitive shape vs the capture-before-discard rule (recorded 2026-07-26, from Opus review of rust/wp2-vfs)

- ~~`crates/rune-core/src/vfs` collapses `Exchange` + temp-unlink into a single `save_atomic`, destroying the displaced bytes *inside* the primitive. No caller can ever re-read and hash what the swap displaced — this forecloses `Materialize` step 4 (the capture-before-discard rule, "capture before discard, guaranteed by mechanism"). The vfs-primitives rule names `Exchange` and `RenameExcl` as two separate primitives with the caller choosing; the internal `path.exists()` choice also cannot express the never-silently-create rule's "never silently create on an overwrite-intent save".~~
- ~~**Resolve before the Phase-2 docstate port**: split the Rust trait into `exchange`/`rename_excl` primitives with the displaced-content temp surviving long enough for the caller to hash/blob it, or add an explicit capture callback to `save_atomic`.~~
- **RESOLVED:** the trait (now `crates/rune-vfs`, not `rune-core/src/vfs` — that path no longer exists) split into `fn exchange(&self, a: &Path, b: &Path)` (`rune-vfs/src/lib.rs:83`) and `fn rename_excl(&self, old: &Path, new: &Path)` (`:89`), backed by `libc::RENAME_SWAP`/`libc::RENAME_EXCL` in `rune-vfs/src/disk.rs:114-120`; `save_atomic` survives only as a demoted default-body convenience (`lib.rs:127`) that says plainly it cannot satisfy the capture-before-discard rule alone. The capture-before-discard this entry demanded is realised at `crates/rune-db/src/materialize.rs:199-223`: it reads and hashes the displaced bytes (`vfs.read(&temp)`, `:201`) and only removes the temp (`vfs.remove(&temp)`, `:223`) after the DB transaction has committed.
...
- `crates/rune-md/tests/conceal_roundtrip.rs` is 1407 lines (file-size budget limit 500). Split into focused test modules with a shared helper crate/module when next touched.
...
- rune-md residual fuzz-line-noise double-claims (4 docs per 214k shipped-build, 182 strict): fence/backtick + blockquote compositions like ">c\n`\n>`" and "t\n  -```\n*```\n>" — focused-only 1-byte caret skews, absorbed by the single-claim chokepoint in shipped builds. Repro/shrinker: scratchpad probe-strict harnesses (stage-2b review). Fix the producer when next in rune-md/parse.
  - **Concrete seed (recorded 2026-08-04, grapheme-width defect-fix session):** `crates/rune-md/tests/conceal_roundtrip_proptest.rs`'s `syntax_map_roundtrip_is_identity_or_clamped_stable` reproduces this class from a random seed. Shrunk input: `content = "-\n  > nested\n  > more\na\r```\na\r```\nplain text"`, `raw_offset = 0`, `focused = false`, failing with `line 7: visible claim [68,69) overlaps 1 already-claimed byte(s) — producer bug (content invented on the visible side)`. Same lone-CR × blockquote × fence composition this entry and the comrak-sourcepos entry below both describe. **Verified pre-existing on `cc204fa`** (the emoji-width work did not introduce it); it is rare enough that six 2048-case runs on a pristine base all passed, so it only surfaces as an intermittent `make test` red. Re-arm it by writing that seed into `crates/rune-md/tests/conceal_roundtrip_proptest.proptest-regressions` — deliberately NOT checked in, since doing so would turn `make test` permanently red for an already-tracked upstream-adjacent bug.
...
- A zero-width edit batch still dirties a clean file: cut with nothing to cut (empty last line / empty buffer) journals a zero-width step, bumps version, and marks the doc dirty — a clean file then warns "unsaved changes" until undo. Fix: skip empty edit batches at the commit chokepoint.
...
## rust port — file-size overages (recorded 2026-07-27)

### recorded 2026-07-29 by the `ts-int` tree-sitter integration merge

- `crates/rune-tui/src/app.rs` is 501 lines (file-size budget limit 500) — one line over.
  ...
  explain where the extracted functions went: the file-size budget is satisfied by splitting a
...
- `crates/rune-tui/src/app.rs` is 509 lines (file-size budget limit 500; was 501 immediately
...
- `crates/rune-md/tests/table_render.rs` is 530 lines (file-size budget limit 500; was 470
...
The file-size budget rule — one primary type per file, decompose any file past 500 lines — is out of scope for the work that surfaced them; recording per the project convention that a pre-existing issue is never silently skipped.
...
- **Root cause:** `rune-db`'s rename path files a replaced file's displaced bytes under the **renaming** document's `doc_id` — its own test asserts `displaced.doc_id == f.ds.doc_id, "captured under OUR doc"` — and blanks any pre-existing `documents` row's path for the replaced file. The constitution's merge-ancestor rule makes the `observations` stream the source `newestObservation`/`ancestorAt` read from — the 3-way-merge baseline. Filing a foreign file's hash and stat under the wrong document's observation stream is therefore a latent wrong-ancestor risk, and a wrong ancestor silently discards a real change (a Catastrophic-severity violation).
...
- **Root cause (as filed):** `full_pipeline_5k_under_100ms` (`crates/rune-md/tests/perf_guard.rs`) permits a 100 ms full display-pipeline run on a 5,000-line document, and `App::sync_view` runs that pipeline synchronously — no async offload — from the runtime's blocking message loop before every frame is drawn. The update loop must stay non-blocking; a 100 ms stall on every keystroke sits uneasily with that.
...
- **Still open, deliberately not addressed by WP16:** the underlying non-blocking-update tension itself — `sync_view` is still a synchronous call inside the blocking loop, and the ORIGINAL `full_pipeline_5k_under_100ms` 100 ms budget is unchanged (a genuinely COLD sync — the first `view()` after opening a huge document, or after an edit large enough to invalidate every memo — still pays close to that full cost, by design; WP16 only removes the cost an ORDINARY keystroke pays needlessly). Moving the recompute itself off the synchronous path (incremental recompute deeper in `rune-md`, or an async worker) remains a larger redesign, out of scope here.

## rust port — RESOLVED (2026-07-28, defect-fix session): Rust corrupted ZWJ/skin-tone emoji grapheme clusters

Root cause: the pipeline built ONE `Cell` per `char`, not per grapheme cluster — a ZWJ sequence's 7 codepoints (or a skin-tone emoji's 2) each got their own buffer cell. `ratatui::buffer::Buffer`'s own diffing (`BufferDiff`, ratatui-core 0.1.2) treats any cell whose `cell_width()` is `> 1` as covering the next `cell_width() - 1` columns too and silently skips re-examining them ("we're assuming buffers are well-formed, that is no double-width cell is followed by a non-blank cell" — its own doc comment); a REAL codepoint sitting in exactly that "covered" column never reached the real terminal, and every later column on the row shifted — the observed reordering/stray-fragment corruption. Fix, in two parts:
...
- **Follow-up (2026-08-04, grapheme-width-vs-ratatui defect-fix session):** the per-rune MAX formula this entry settled on was itself still wrong for a class of clusters this fix never covered — a base char plus a variation selector (`U+FE0F`/`U+FE0E`), a regional-indicator flag pair, a keycap, and halfwidth katakana + dakuten all disagree with what ratatui's own `CellWidth for str` derives for the same string (`ratatui-core`'s `impl CellWidth for str` uses `unicode_width::UnicodeWidthStr::width` over the WHOLE cluster, not a per-rune MAX). `grapheme_width` now measures the whole cluster with `UnicodeWidthStr::width` instead — this is NOT the summing this entry rejected above (summing was rejected because it overcounts a ZWJ/skin-tone cluster; `UnicodeWidthStr::width`'s ligature-aware rule still lands on the same width confirmed for those clusters, while fixing the variation-selector/flag/keycap/katakana cases the MAX rule got wrong). See `crates/rune-syntax/src/wrap/width.rs`'s `grapheme_width` doc comment for the corpus and the guard test in `crates/rune-tui/tests/tui_render_text.rs` that pins the two derivations equal.

## rust port — RESOLVED (2026-07-28, defect-fix session): Rust rendered no checkbox glyph for GFM task list items

Root cause: `rune-md/src/emit/walk.rs`'s `emit_list_item` `hide_range`d a task item's WHOLE marker (the `"- [ ] "` prefix, including the checkbox itself) when concealed, with nothing substituted in its place — the `StyleId::TaskMarker` style existed (used by the revealed/raw path) but nothing ever produced a `Substituted` span for it. Fix: `emit_list_item`'s concealed branch now hides only the `"- "`/`"1. "` prefix before the checkbox (same as a plain marker) and calls a new `push_task_checkbox`, which substitutes the item's own `task: ByteRange` (always exactly the 3 ASCII bytes `"[ ]"`/`"[x]"`/`"[X]"`, per `ListItemM::task`'s docs) with `☐` (U+2610) or `☑` (U+2611) — each also exactly 3 UTF-8 bytes, so the substitution is byte-length-neutral by construction and needs no extra `SyntaxSnapshot` hidden-range bookkeeping (that coordinate model only accounts for FULLY hidden byte ranges, not length-changing substitutions). The trailing space(s) between the checkbox and the item's own text are deliberately left unclaimed for `fill_gaps` to supply verbatim, so 0/1/N trailing spaces round-trip exactly instead of a hardcoded second space. Updated `rune-md/tests/conceal_roundtrip.rs`'s `tasklist_marker_conceals_off_cursor_line` to assert the new `"☑ task"` output.

## rust port — accepted deviation: word motion classifies Unicode letters as `Word` (recorded 2026-07-28, editor-MVP plan WP9.S1)

**Status:** deliberate design decision, not a defect — recorded per plan WP9.S1's explicit instruction.

`crates/rune-tui/src/commands/nav.rs`'s `char_class` classifies a rune as `Word` when `unicode-segmentation`'s UAX #29 word-boundary algorithm (probed via `is_word_forming`) would fuse it into a word alongside ordinary letters — not just ASCII `[A-Za-z0-9_]` — so `⌥←`/`⌥→` behave correctly over any script (Cyrillic, Greek, CJK ideographs, combining marks, …) instead of stopping at every individual non-ASCII character. Whitespace is likewise classified via `char::is_whitespace()` rather than an ASCII-only check. No action needed; recorded so the choice reads as considered, not an unreviewed gap.
...
- **Approximate caret placement inside a rendered table row.** A table line's row 1
  ...
  placement on a table row is therefore approximate, and unreachable in the focused
  pane in practice (reveal is whole-block — decision 5 — so a rendered table row is
  never the row the cursor is actually drawn on).
- **`crates/rune-md/tests/table_render.rs`** is 495 lines (file-size budget limit 500) — 458 after WP6's
  ...
  sibling files over the limit (see the "file-size overages" entry above and the
...
**Status:** fixed, with a regression test; found while exercising a Wrapped table through the full render -> wrap -> display pipeline for the first time (WP4's own tests covered segment `start_col`s and cell wrapping, but not `DisplaySnapshot`'s border synthesis for a Wrapped line specifically).

- **Symptom:** a Wrapped table's body row, once its cells wrap into more than one visual sub-row, rendered a `└┴┘` bottom border after EVERY sub-row instead of only the last one — and that border (and the table's top border) was drawn at the Grid layout's natural, unshrunk column widths instead of the actually-rendered, proportionally-shrunk Wrapped widths, so the border ran wider than the content it bordered and overflowed the pane. Confirmed via a real `tmux capture-pane` capture, not just a unit test.
...
The check itself is gated on `rune-core`'s `STRICT_INVARIANTS` (the never-panic rule: an
...
- **Fix design:** hand-parse `![[` in the same pass, carrying an explicit
  `Embed` flag, or pre-scan for the construct before comrak sees it.
...
## rust port — file-size budget: `rune-syntax/src/wrap/mod.rs` grew past the ceiling fixing CELL-ORDER (recorded 2026-07-29)

- `crates/rune-syntax/src/wrap/mod.rs` is 587 lines (file-size budget limit 500; was 489
...
## rust port — file-size budget: `rune-cli/src/main.rs` and `explorer.rs` grew in the navigation plan (recorded 2026-07-29)

- `crates/rune-cli/src/main.rs` is 521 lines (file-size budget limit 500; was 429 before this
...
- `crates/rune-tui/src/explorer.rs` is 588 lines (file-size budget limit 500; was 553
...
model, bytes not cells) while `comrak_line_starts` follows CommonMark, where a bare CR
...
over the line break, and the `\r` survives alone (paste is verbatim by the
write-bytes-verbatim rule, correctly). Opening a CR-only file does the same. The render layer maps `\r` to
...
a parse-time view, not a mutation, so the write-bytes-verbatim rule is untouched. The dual index then
ceases to exist and the mis-projection becomes unrepresentable. It changes what
`x\r| a | b |` parses to — a user-visible behavior change that deserves its own
scrutiny before committing — that is why it is not being done inside the
tree-sitter plan.
...
**`crates/rune-tui/tests/tui_render.rs` is now 555 lines**, over the file-size budget. It was
...
invariant, same architectural root cause as the ZWJ/skin-tone emoji grapheme-cluster
fix above (Grid's `col_widths` measuring a cell's JOINED text instead of its rendered
...
this is a live bytes-vs-cells hypothesis worth checking first.~~
...
round), well over the file-size budget. It is the largest file in that change set. Split it
...
## rune-db — file-size overages (recorded 2026-07-29, code-review WP6)

Six files in `crates/rune-db/src` are over the file-size 500-line ceiling. None of
...
over the file-size budget. Adding the `alias` field to `Binding` cost one line per literal and
