...
- **Follow-up (2026-08-04, grapheme-width-vs-ratatui defect-fix session):** the per-rune MAX formula this entry settled on was itself still wrong for a class of clusters this fixture never covered — a base char plus a variation selector (`U+FE0F`/`U+FE0E`), a regional-indicator flag pair, a keycap, and halfwidth katakana + dakuten all disagree with what ratatui's own `CellWidth for str` derives for the same string (`ratatui-core`'s `impl CellWidth for str` uses `unicode_width::UnicodeWidthStr::width` over the WHOLE cluster, not a per-rune MAX). `grapheme_width` now measures the whole cluster with `UnicodeWidthStr::width` instead — this is NOT the summing this entry rejected above (summing was rejected because it overcounts a ZWJ/skin-tone cluster; `UnicodeWidthStr::width`'s ligature-aware rule still lands on the same width THIS entry's tmux capture confirmed for those clusters, while fixing the variation-selector/flag/keycap/katakana cases the MAX rule got wrong). See `crates/rune-syntax/src/wrap/width.rs`'s `grapheme_width` doc comment for the corpus and the guard test in `crates/rune-tui/tests/tui_render_text.rs` that pins the two derivations equal.
