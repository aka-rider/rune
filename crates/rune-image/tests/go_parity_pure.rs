//! Byte-parity: every pure function in `rune-image` reproduces the Go
//! reference implementation's outputs exactly, verified against
//! `tests/golden/pure.json` (regenerated by `make image-parity-dump`).
//!
...
fn load_golden() -> Value {
    ...
    let text = fs::read_to_string(path).unwrap_or_else(|e| {
        panic!("read {path}: {e} (run `make image-parity-dump` from the repo root)")
    });
