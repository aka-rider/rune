...
fn content_end(text: &str, run: &[CellSrc]) -> Option<usize> {
    ...
    for (ch, src) in text.chars().zip(run.iter()) {
        if let Some(buf) = src.buf {
            end = Some(buf as usize + ch.len_utf8());
        }
    }
    end
}
...
pub fn row_spans(
    ...
    for (i, (text, run, scope)) in runs.iter().enumerate() {
        ...
        let cell_map = run.iter().map(|c| c.buf).collect();
        spans.push(SyntaxSpan::substituted_mapped(
            *scope,
            text.clone(),
            s..e,
            cell_map,
...
pub fn extra_row_spans(
    ...
    runs.iter()
        .map(|(text, run, scope)| {
            let cell_map = run.iter().map(|c| c.buf).collect();
            SyntaxSpan::substituted_mapped(*scope, text.clone(), line_start..line_start, cell_map)
        })
        .collect()
}
...
mod tests {
    ...
    fn worked_example_tiles_exactly_as_the_plan_specifies() {
        let runs: Vec<(String, Vec<CellSrc>, ScopeId)> = vec![
            ("| ".to_string(), vec![src(None), src(None)], ScopeId(1)),
            ("a".to_string(), vec![src(Some(2))], ScopeId(2)),
            (" ".to_string(), vec![src(None)], ScopeId(2)),
            (
                " | ".to_string(),
                vec![src(None), src(None), src(None)],
                ScopeId(1),
            ),
            (
                "bb".to_string(),
                vec![src(Some(6)), src(Some(7))],
                ScopeId(2),
            ),
            (" |".to_string(), vec![src(None), src(None)], ScopeId(1)),
        ];
    ...
    fn fully_decorative_row_tiles_into_one_span_covering_the_whole_line() {
        let runs: Vec<(String, Vec<CellSrc>, ScopeId)> =
            vec![("├───┤".to_string(), vec![src(None); 5], ScopeId(3))];
    ...
    fn extra_row_spans_all_carry_the_same_empty_range_at_line_start() {
        let runs: Vec<(String, Vec<CellSrc>, ScopeId)> = vec![
            ("  Name: ".to_string(), vec![src(None); 8], ScopeId(1)),
            (
                "Alice".to_string(),
                vec![
                    src(Some(10)),
