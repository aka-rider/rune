...
//! to keep that module (and this crate's `parse/mod.rs`) under
...
fn image_between_words_is_not_a_standalone_line() {
    ...
    assert!(standalone_image(content, &p.inlines).is_none());
}
...
fn list_item_image_is_a_standalone_line() {
    ...
    assert!(found.is_some(), "expected a standalone image");
    assert_eq!(text_of(content, found.unwrap().target), "x.png");
}
...
fn whitespace_padded_image_line_is_standalone() {
    ...
    assert!(standalone_image(content, &p.inlines).is_some());
}
