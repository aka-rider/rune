mod palette_cmdpal;
mod palette_doc;
mod palette_find;
mod palette_input;

pub(super) use palette_cmdpal::{
    CMDPAL_BACKSPACE_KEY, CMDPAL_KEY_CTRL, CMDPAL_KEY_SUP, CMDPAL_NAV_KEYS, CMDPAL_PARAM_QUERIES,
    CMDPAL_TAB_KEY, MERGE_KEY, MERGE_RESOLVE_KEYS,
};
pub use palette_doc::TYPE_PALETTE;
pub(super) use palette_doc::{MARKDOWN_FRAGMENTS, PASTE_PALETTE, SEEDS};
pub(super) use palette_find::{
    FIND_CHARS, FIND_KEY_CTRL, FIND_PANEL_KEYS, PROJECT_FIND_KEY_CTRL, PROJECT_REPLACE_KEY_CTRL,
    REPLACE_ALL_KEY, REPLACE_KEY_CTRL, RESULTS_KEYS,
};
pub(super) use palette_input::{
    ADD_CURSOR_ABOVE_KEY, ADD_CURSOR_BELOW_KEY, COPY_KEY, CTRL_B_KEY, CTRL_C_KEY, CTRL_E_KEY,
    CTRL_P_KEY, CTRL_T_KEY, CUT_KEY, DELETE_KEYS, ENTER_KEY, ESCAPE_KEY, EXPLORER_SEARCH_KEYS,
    F2_KEY, FILESEARCH_KEY_CTRL, FILESEARCH_KEY_SUP, NAV_BACK_KEY, NAV_FORWARD_KEY, NAV_KEYS,
    PASTE_KEY, REDO_KEY, REDO_KEY_ALT, SAVE_KEY, SELECT_ALL_KEY, SELECT_MOTION_KEYS,
    TITLE_MOTION_KEYS, TRASH_KEY, UNDO_KEY,
};
