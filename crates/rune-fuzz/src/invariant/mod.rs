...
pub fn check_all(prev: &Snapshot, next: &Snapshot, ctx: &StepCtx) -> Option<Violation> {
    cur_bounds(next)
        ...
        .or_else(|| cur_id(next))
        .or_else(|| cur_no_caret_hidden(&next.painted))
        ...
        .or_else(|| table_row_width(&next.painted))
        .or_else(|| table_synthetic_decorative(&next.painted))
        ...
        .or_else(|| hl_clamped(&next.painted))
