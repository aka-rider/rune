...
proptest! {
    ...
    fn snapshot_immutability(
        ...
        // `start`/`end` are in-bounds but not necessarily on a char
        // boundary — `replace` correctly refuses a split (`SplitsRune`);
        // the immutability property under test only applies when the edit
        // actually goes through.
        if let Ok(new_b) = b.replace(start, end, &insert) {
    ...
    fn batch_equivalence(
        ...
        let b = Buffer::new(init);
        // `s1`/`e1`/`s2`/`e2` are in-bounds but not necessarily on char
        // boundaries — either the sequential or the batch path (or both)
    ...
    fn point_roundtrip(
        ...
        let start = b.line_start(line).expect("line is in-range by construction");
        let end = if line == b.line_count() - 1 {
            b.len()
        } else {
            b.line_end(line).expect("line is in-range by construction")
        };
        let raw = raw_col % (end.saturating_sub(start) + 1);
        // Snap the generated BYTE column to a char boundary before asserting
        // the round trip. A mid-UTF-8 column is representable but is not a
        ...
        // The stronger property the snapping actually buys: whatever byte
        // column goes in, the offset that comes out is always a valid char
        // boundary — so no caller can mint an out-of-boundary cursor.
        let raw_offset = b.line_col_to_offset(BufferPoint { line, col: raw });
        prop_assert!(b.content().is_char_boundary(raw_offset));
    }
