...
fn up_and_down_clamp_at_the_list_bounds() {
    ...
    let mut session = open_seeded(&mem);
    drive_load_explorer(&mut session);
    // `drive_load_explorer`'s `^b` now reveals the active document's own
    // file (the Enter/Escape rework's "land on the active file" contract),
    // not necessarily row 0 — pin a known starting row before probing the
    // clamp itself, which is what this test is actually about.
    session.app_mut().explorer.nav.cursor = 0;
    let mut effects = Effects::default();

    assert_eq!(
        explorer_keys::handle_key(session.app_mut(), key(KeyCode::Up), &mut effects),
        KeyOutcome::Consumed
    );
    assert_eq!(session.app().explorer.nav.cursor, 0, "clamped at the top");
    ...
    assert_eq!(
        session.app().explorer.nav.cursor,
        session.app().explorer.entries.len() - 1,
...
fn enter_on_a_non_utf8_named_file_opens_the_byte_exact_path() {
    ...
    assert_eq!(
        session.app().explorer.entries[idx].path,
        raw_path,
...
fn enter_on_a_directory_issues_a_read_dir_cmd() {
    ...
    assert!(session.app().explorer.loading);
    assert_eq!(
...
fn enter_on_the_parent_row_loads_the_parent_directory() {
    ...
    assert_eq!(session.app().explorer.entries[0].name, "..");
    // `drive_load_explorer`'s `^b` now reveals the active document's own
    // file, not necessarily row 0 (see `up_and_down_clamp_at_the_list_
    // bounds`'s own note) — this test is about the ".." row specifically,
    // so park the cursor there explicitly.
    session.app_mut().explorer.nav.cursor = 0;
    ...
    assert_eq!(session.app().explorer.root, PathBuf::from("/"));
}
