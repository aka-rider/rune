...
//! collision guard and both halves of hazard 1), and `rename_replace.rs`
//! (the `[R]eplace` path against a real in-memory `Store`) — TODO.md's
//! §1.6 split of the original `rename.rs`. Each consumer pulls this in via
//! `mod rename_common;` — integration test files are separate binaries,
//! so this is the one place all three draw an identical `App`/`Mem`
