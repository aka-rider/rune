...
/// `F2` then select-all-equivalent: clear the STEM (the extension is fenced
...
/// `F2`, clear the stem, then type `name` — WITHOUT pressing Enter, so the
