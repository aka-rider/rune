...
/// Rendering a fenced markdown document leaves every cell inside the fence
/// carrying a non-`None` background — the `markup.raw.block` fence
/// background survives the highlight overlay's `patch`, since the overlay
/// scope style sets only `fg` and never `.bg(..)`.
#[test]
