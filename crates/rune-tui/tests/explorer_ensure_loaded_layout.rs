...
/// A frame too narrow to fit the left column at all resolves to
/// `LayoutMode::EditorOnly` even though `app.splits.left` still says shown
/// (`layout::resolve_mode`'s own contract) — `ensure_loaded` must honor
/// that and request nothing.
#[test]
fn a_frame_too_narrow_to_paint_the_explorer_requests_no_listing() {
    let mem = seeded_vfs();
