...
//! original `rename.rs`. The collision guard/hazard-1 tests and the
//! store-backed `[R]eplace` path live in the sibling
//! `rename_collision.rs`/`rename_replace.rs`; all three pull shared
