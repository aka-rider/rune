...
#[test]
fn a_failed_load_detaches_the_replica_so_later_edits_are_not_buffered_forever() {
    let mem = Mem::new();
    mem.save_atomic(Path::new("/doc.md"), b"hello")
        .expect("seed doc.md");
    let vfs: Arc<dyn Vfs + Send + Sync> = Arc::new(mem);
    let mut app = App::new(Buffer::new(""), None, vfs, Some(in_memory_db()));
    let id = crate::workspace::open_path(&mut app, Path::new("/doc.md")).expect("doc.md opens");
    assert!(
        matches!(
            app.doc(id).expect("doc exists").replica,
            Replica::Binding { .. }
        ),
        "test setup: the open-time load is in flight"
    );
    let op_id = *app
        .db_ops
        .iter()
        .find(|(_, pending)| pending.doc == id && pending.issued_version.is_some())
        .expect("the load op is recorded")
        .0;

    let mut effects = crate::runtime::Effects::default();
    crate::app::update(
        &mut app,
        crate::runtime::Msg::Db(DbEvent::Err {
            id: op_id,
    ...
    assert!(
        matches!(app.doc(id).expect("doc exists").replica, Replica::Detached),
        "a failed load must end the binding state, not leave it waiting for an ack that never comes"
    );
    assert_eq!(
        messages::newest_text(&app),
    ...
    crate::app::update(
        &mut app,
        crate::runtime::Msg::Key(crate::keymap::KeyInput {
    ...
    assert_eq!(app.doc(id).expect("doc exists").buffer.content(), "xhello");
    assert!(
        matches!(app.doc(id).expect("doc exists").replica, Replica::Detached),
        "an edit after the failed load must not be buffered for the store"
    );
}
