...
use rune_db::{MergePrepResult, SyncKind};
...
pub(crate) fn handle_merge_prep_ack(
    ...
    let Ok(theirs_text) = String::from_utf8(prep.theirs.clone()) else {
        app.merge = MergeState::Inactive;
    ...
    if blocks.is_empty() || intent == MergeIntent::Discard {
        let target = if intent == MergeIntent::Discard {
            &theirs_text
        } else {
            &buffer_text
        };
        if !install_whole_range(app, doc, target) {
            app.merge = MergeState::Inactive;
            app.set_status(
        ...
        enqueue_resolve_adopt(app, doc, prep.theirs_obs);
        app.merge = MergeState::Inactive;
        let message = match intent {
            MergeIntent::Discard => "disk changes adopted",
            MergeIntent::Merge => "merged cleanly — disk changes applied",
        };
        app.set_status(message, StatusSource::Other);
        return;
    }

    if !install_whole_range(app, doc, &buffer_text) {
        app.merge = MergeState::Inactive;
        app.set_status(
            "merge failed — the document could not be updated",
            StatusSource::Other,
...
fn install_whole_range(app: &mut App, doc: DocumentId, text: &str) -> bool {
    ...
    commit_edit_batch(app, doc, vec![(edit, 0)], cursors_before)
}
...
fn enqueue_resolve_adopt(app: &mut App, doc: DocumentId, theirs_obs: rune_db::ObsId) {
