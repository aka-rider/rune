...
impl FileSearchState {
    /// Resolves a `ResultRow::candidate_idx`/flat listing index into the
    /// candidate it names: `recents` occupies the low indices, `walk` the
    /// rest — the same flat addressing `recompute`'s empty-query listing
    /// builds, so recents always sort ahead of walk results at equal rank.
    /// Unused outside `#[cfg(test)]` until a later work package's render
    /// pass reads it too.
    #[allow(dead_code)]
    pub(crate) fn candidate(&self, idx: usize) -> Option<&Candidate> {
        match self.recents.get(idx) {
            Some(candidate) => Some(candidate),
            None => self.walk.get(idx - self.recents.len()),
        }
    }
}
