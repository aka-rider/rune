//! Terminal graphics capability: whether the Kitty graphics
...
//! and re-derived on every `Msg::Resize` (`runtime::apply`). The
//! image document's decode/re-fit pipeline sits on top of this: `footprint`
//! (pure fit-to-width math), `decode_cmd` (the decode `Cmd`, its
//! scheduling chokepoint, and the reply handler), and `resize_refit` (the
//! `Msg::Resize`-driven re-fit/retransmit). `embed` reuses the same
...
/// teardown-time Kitty flag in sync with it — one
