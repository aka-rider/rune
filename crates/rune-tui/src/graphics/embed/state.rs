//! `EmbedState`/`EmbedSet`: the per-embed lifecycle state a
//! markdown `Document` tracks for every live `![alt](x.png)`/`![[x.png]]`
//! it contains, keyed by `ImageM::target_text` — the same raw string
//! `rune_md::snapshot::ImageDims` and `SnapshotRow::image`'s own
//! `ImageRowRef::target` key off, so the renderer, the
...
pub struct EmbedState {
    ...
    /// sentinel: `Failed` is sticky per `(path, mtime)`.
...
/// A markdown document's whole embed set: every
...
pub struct EmbedSet {
    ...
    /// mirroring `ImageState::in_flight`'s generation shape —
    ...
    /// itself needing to carry a target key (kept off `runtime/mod.rs`,
    /// which is already at its 500-line budget ceiling).
