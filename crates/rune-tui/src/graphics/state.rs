...
/// and dropped; `pending` records that this image needs re-decoding once an
/// in-flight decode finishes (the same `in_flight`/`pending` shape
/// `HighlightState` already uses).
