//! `help_markdown` — the Help virtual document's content, generated from
//! the real binding tables so the Help doc and each pane's
...
//!
//! The `## Editor` section is generated by this same reflection pass over
//! `editor_bindings::EDITOR_BINDINGS`, not hand-maintained — there is no
//! separate key list to fall out of sync with `keymap::resolve`'s match
//! arms.
...
mod tests {
    ...
    /// The generated `## Editor` section's row count must equal
    /// `EDITOR_BINDINGS.len()` exactly — reflection, not a hand-authored
    /// count that could silently drift from the table.
    #[test]
    ...
    /// The generated Help doc must contain the reload
