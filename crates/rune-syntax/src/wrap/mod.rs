...
use crate::syntax::{RowBoundary, SyntaxLine, SyntaxSpan, TableRole, TableRowInfo};
...
/// more than one row-1 segment — WP4's Wrapped/Pivoted layout is what
/// makes a single source line's `extra_rows` produce more than one
/// segment). `DisplaySnapshot` (WP3) reads this to decide where to
/// synthesize a top/bottom/inter-row border row.
#[derive(Clone, Debug)]
...
impl WrapMap {
    ...
    /// A table source line's row 1 is `line.spans` (already tiled by
    /// `table::row_spans`, WP2.S5) and never re-wrapped — Grid layout (WP2)
    ...
    fn wrap_line(
        ...
        if let Some(info) = &line.table {
            self.wrap_table_line(line_idx, line, info, segments);
            return;
        }
