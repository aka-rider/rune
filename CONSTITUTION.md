...
- **§1.4.1 Atomic durable writes.** Write user content only through a durable temp write (`vfs.Disk.WriteFile`: create → write → fsync data) followed by an atomic publish to the destination — `vfs.Exchange` (overwrite, `renamex_np` RENAME_SWAP) or `vfs.RenameExcl` (create, `renamex_np` RENAME_EXCL), each fsyncing the parent directory after the rename. The buffer is clean only after the publish (the swap/rename) returns.
...
- [ ] User-content writes go through a durable temp write + atomic `vfs.Exchange`/`RenameExcl` publish; unsaved work goes to the recovery store, never a debounced write to the destination. (§1.4.1, §1.4.2)
...
- **Tree-sitter output is a render-layer overlay, never an emitted `SyntaxSpan`** (Rust port only — `rune-ts::highlight`, patched onto cell styles by `render::overlay::apply_highlight_spans` via `cell.style = cell.style.patch(..)`). A token re-tagging bytes the emitter already claimed for something else (e.g. `markup.raw.block`) is unrepresentable through emit, and an overlay survives the emitter's unconditional recompute on every edit; a second emitted layer would not.
- **Nested captures paint outer-first, never a per-cell search.** `apply_highlight_spans` sorts spans `(start ASC, end DESC, capture order ASC)` and paints them in that order into a byte-indexed window sized to the visible bytes — the innermost, latest-painted span wins, matching `tree-sitter-highlight`'s own last-pattern-wins resolution with no per-cell lookup.
- **`DocumentKind` selects the producer; there is no second plain-text producer.** A `Code`/`Plain` document (`rune_syntax::kind::DocumentKind`) reuses `rune_md::emit::emit` with an empty block list — its gap-fill pass already yields one verbatim `Identical` span per line — skipping only the comrak parse. `Image` is a genuine third producer (plan WP4): it skips emit/wrap entirely and synthesizes its `DisplaySnapshot` directly (`DocMachine::rebuild`'s `Image` branch calling `DisplaySnapshot::image_rows`) — an image document's buffer is always empty, so there are no lines to gap-fill.
- **The parse is whole-region and retained; the query is viewport-scoped and stateless.** A viewport-keyed *cache* would be invalidated by scrolling — a second source of truth needing re-timing — and there is none. The single stored artefact per code region is its `rune_ts::ParsedTree` (`RegionHighlight::tree`), parsed from that region's whole prefix-free source text under a span cap (`rune_ts::MAX_SPANS`) and a `PassBudget` — an undivided per-region deadline (`PARSE_BUDGET`) paired with a ceiling every region of one pass shares (`PASS_BUDGET`). No budget is ever divided by region count: a lone expensive region gets the whole per-region cap, and once the pass ceiling is spent the regions not yet reached carry forward untouched and are picked up by the reschedule. `PassBudget` has no constructor that omits a total, so an unbounded pass is unrepresentable rather than merely unused; the synchronous pre-first-draw pass is the same mechanism with `FIRST_PAINT_BUDGET` as both cap and ceiling. Every frame, `render::build_rows` derives the visible byte range and calls `highlight::visible_spans`, which queries each intersecting region's retained tree over just that window (`rune_ts::highlight_range`) and stores nothing — so scrolling invalidates nothing, and a region is reparsed only when its own bytes change.
- **At most one highlight runs per document.** `Document::highlight` (`HighlightState`) tracks `in_flight`/`pending`; a version change while a highlight runs arms `pending` rather than spawning a second thread. Both levels of `None` in the reply mean "carry forward", never "clear": `Msg::Highlighted { result: None, .. }` — budget elapsed, unknown language, or parse failure — leaves every region's colours untouched, and within a `HighlightReply`, a `RegionResult` whose `payload` is `None` (its retained tree was still valid, or its reparse overran the budget) takes the refreshed `LineMap` while inheriting the channels that region already had (`highlight::install_regions`). A payload that IS present replaces that region's channels outright, including with an empty span list ("parsed, no tokens"). Without that distinction a document whose parse exceeds the budget would lose its colours on every keystroke and never regain them.
