...
- **Highlighting is whole-document with a parse budget, never viewport-keyed.** A viewport-keyed cache would be invalidated by scrolling — a second source of truth needing re-timing. `rune_ts::highlight` is bounded by a deadline (`HIGHLIGHT_BUDGET`) and a span cap (`MAX_SPANS`) instead.
- **At most one highlight runs per document.** `Document::highlight` (`HighlightState`) tracks `in_flight`/`pending`; a version change while a highlight runs arms `pending` rather than spawning a second thread. `Msg::Highlighted { result: None, .. }` means "no result" — budget elapsed, unknown language, or parse failure — and leaves the previously stored spans untouched, distinct from `Some(vec![])` ("parsed, no tokens"); without that distinction a document whose parse exceeds the budget would lose its colours on every keystroke and never regain them.
