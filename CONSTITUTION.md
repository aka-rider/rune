...
- **The parse is whole-region and retained; the query is viewport-scoped and stateless.** A viewport-keyed *cache* would be invalidated by scrolling — a second source of truth needing re-timing — and there is none. The single stored artefact per code region is its `rune_ts::ParsedTree` (`RegionHighlight::tree`), parsed from that region's whole prefix-free source text under an undivided per-region deadline (`PARSE_BUDGET`) and a span cap (`rune_ts::MAX_SPANS`); the one synchronous pre-first-draw pass uses `FIRST_PAINT_BUDGET` instead, and is the only place a budget is divided across regions. Every frame, `render::build_rows` derives the visible byte range and calls `highlight::visible_spans`, which queries each intersecting region's retained tree over just that window (`rune_ts::highlight_range`) and stores nothing — so scrolling invalidates nothing, and a region is reparsed only when its own bytes change.
