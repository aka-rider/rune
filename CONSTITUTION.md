...
- **`DocumentKind` selects the producer; there is no second plain-text producer.** A `Code`/`Plain` document (`rune_syntax::kind::DocumentKind`) reuses `rune_md::emit::emit` with an empty block list — its gap-fill pass already yields one verbatim `Identical` span per line — skipping only the comrak parse.
